...
class _SeatSelectionPageState extends State<SeatSelectionPage>
  ...
  bool _isFamilyPackage = false;
  bool _showLegend = true;
  bool _isLoadingSeats = true;
  bool _isBooking = false;

  int get _maxSeats => _isFamilyPackage ? 5 : 3;
  String? _seatsError;
  DateTime? _expiresAt;
  ...
  Future<void> _onSeatTap(_SeatCell seat) async {
    ...
    if (result != null && mounted) {
      setState(() {
        ...
        seat.passengerName = result.passengerName;
        seat.contactNumber = result.contactNumber;
    ...
    try {
      ...
      final payload = sel
          .map((s) => {
                ...
                'gender': _genderStr(s.gender ?? SeatGender.male),
      ...
      final booking = await repo.createBooking(
          tripId: _bus.id,
          seats: payload,
          familyPackage: _isFamilyPackage,
          offerId: widget.params.offerId);
      if (!mounted) return;
      ...
      final data = BookingData(
        ...
        selectedSeats: sel
            .map((s) => SelectedSeat(
                  seatId: s.id,
                  seatNumber: s.number,
                  gender: s.gender ?? SeatGender.male,
                ...
                : SingleChildScrollView(
                    ...
                    child: Column(
                        ...
                        children: [
                          ...
                          _buildFamilyPackageCard(tt, cs, isDark),
                          if (_showLegend) ...[
  ...
  Widget _buildFamilyPackageCard(TextTheme tt, ColorScheme cs, bool isDark) {
    final card = isDark ? AppColors.darkSurface : AppColors.white;
    final border = isDark ? AppColors.darkOutline : AppColors.lightOutline;
    final activeBorder = const Color(0xFFF59E0B);
    final subtle =
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(12),
        border: Border.all(
          color: _isFamilyPackage ? activeBorder : border,
          width: _isFamilyPackage ? 1.5 : 1,
        ),
      ),
      child: Row(children: [
        Container(
        ...
        Expanded(child: Column(
          ...
          children: [
            ...
            Text(
              _isFamilyPackage
                  ? 'Up to 5 seats enabled'
                  : 'Enable to book up to 5 seats',
        ...
        Switch(
          ...
          onChanged: (val) {
            setState(() {
              ...
              // If turning OFF while more than 3 seats are selected, deselect the extras
              if (!val) {
                final sel = _selected;
                if (sel.length > 3) {
                  for (final s in sel.skip(3)) {
                    s.display = _SeatDisplay.available;
                    s.passengerName = '';
                    s.contactNumber = '';
                    s.gender = null;
                    s.hasLuggage = false;
  ...
  Widget _buildBottomPanel(
    ...
    final basePrice = sel.length * _bus.pricePerSeat;
    final discount = _isFamilyPackage ? basePrice * 0.1 : 0.0;
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        if (_isFamilyPackage) ...[
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Text('Subtotal (${sel.length} seats):',
                style: tt.bodySmall?.copyWith(color: subtle)),
            Text('GH₵ ${basePrice.toStringAsFixed(2)}',
                style: tt.bodySmall?.copyWith(
                    color: subtle,
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Text('Family Discount (10%):',
                style: tt.bodySmall?.copyWith(color: const Color(0xFF16A34A))),
            Text('- GH₵ ${discount.toStringAsFixed(2)}',
                style: tt.bodySmall?.copyWith(
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text('Estimated Total:',
              style: tt.bodyMedium?.copyWith(fontWeight: FontWeight.w700)),
          Text('GH₵ ${finalPrice.toStringAsFixed(2)}',
        ...
        SizedBox(
          ...
          child: ElevatedButton(
            ...
            child: _isBooking
                ...
                : Text(
                    'Reserve ${sel.length} Seat${sel.length > 1 ? "s" : ""} · GH₵ ${finalPrice.toStringAsFixed(2)}'),
          ),
        ),
      ]),
    );
        ...
        child: SingleChildScrollView(
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text('Seat ${widget.seatNumber} Details',
                style: tt.titleMedium?.copyWith(fontWeight: FontWeight.w700)),
            const SizedBox(height: 4),
            Text('Enter passenger info, gender & luggage',
                style: tt.bodySmall?.copyWith(color: subtle),
            ...
            if (_hasLuggage) ...[
              ...
              Row(children: [
                _luggageChip(
                    'backpack', 'Backpack', Icons.backpack_rounded, border),
                const SizedBox(width: 8),
                _luggageChip(
                    'suitcase', 'Suitcase', Icons.luggage_rounded, border),
              ]),
            ],
            const SizedBox(height: 24),
